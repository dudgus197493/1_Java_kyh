...
import edu.kh.poly.ex2.model.vo.KYHCalculator;
...
public class AbstractionService {
	...
	public void ex1() {
		...
		Fish f1 = new Fish();
		f1.setType("어류");
		f1.breath(); // **동적 바인딩이 아님**, 자식 객체내부에 부모 부분이 있음을 확인
		
		Person p1 = new Person();
		p1.setType("포유류"); // 자식 객체내부에 부모 부분이 있음을 확인
		
		// 추상 클래스는 부모 타입의 참조 변수로 사용하 수 있다.
		// - 다형성의 업 캐스팅
		Animal a1 = new Fish(); 	// 업 캐스팅 
		Animal a2 = new Person();	// 업 캐스팅
		
		// 추상 메서드였던 breath() 호출 가능 여부 확인
		a1.breath();	// void edu.kh.poly.ex2.model.vo.Animal.breath() : 실행전 정적 바인딩 - Animal
		a2.breath();	// 실행 중 오버라이딩한 자식의 breath()로 연결됨(동적 바인딩)
						// -> 동적 바인딩에 의해 정상 수행 될 것을 알기 때문에 에러 발생 X
		
		
		// 추상 클래스의 적용
		// 객체배열 + 추상 클래스
		Animal[] arr = new Animal[3];	// Animal 참조변수 3개
	...
	public void ex2() {
		...
		for(int i =0; i<arr.length; i++) {
			arr[i].lesson();  // 정적 바인딩 : KH.lesson() (추상 메서드) 
							  // 동적 바인딩 : 
							  // arr[0] -> HSH.lesson();
							  // arr[1] -> HGD.lesson();
		}
	...
	public void ex3() {
		Calculator cal = new KYHCalculator();
