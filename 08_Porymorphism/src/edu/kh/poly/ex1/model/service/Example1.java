...
public class Example1 {
	...
	public void ex1() {
		...
		c2.setSeat(2);
		c2.setFuel("휘발유");
		// c2.setWeight(1.5);	// The method setWeight(double) is u.ndefined for the type Car
...
//		System.out.println(c1.toString());
		...
		// 동적 바인딩 : 프로그램 실행 중
		//            참조하는 객체의 실제 타입의 오버라이딩된 메서드랑 연결
	...
	// 다형성이 없다면 출력하는 멕소드를 각각 따로 만들어야 함
...
//	}
	...
	public void ex3() {
		...
		// (Spark)c1.dc();
		// .연산자와 (형변환 연산자)중 ()우선순위가 제일 늦기 때문에 .연산자가 먼저 실행되어 Car참조변수는 dc()메서드르 사용할 수 없음
		// ()연산자에게 ()를 한번더 씌워서 먼저 실행할것을 명시해줘야 함
		((Spark)c1).dc();
		// 형변환 먼저 진행 후 dc() 호출
		// -> 다운 캐스팅
		...
		System.out.println(c3 instanceof Spark); // false
	...
	public void check(Car c) {
		...
		if(c instanceof Spark) {		// 참조하는 객체가 Spark인 경우
			((Spark)c).dc();			// Spark로 다운캐스팅 후 메서드 호출
		}else if(c instanceof Truck) {	// 참조하는 객체가 Truck인 경우
			((Truck)c).loading();		// Truck으로 다운캐스팅 후 메서드 호출
