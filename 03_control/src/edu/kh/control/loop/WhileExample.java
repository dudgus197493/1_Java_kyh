package edu.kh.control.loop;

import java.util.Scanner;

public class WhileExample {
	
	/* while문
	 * 
	 * - 별도의 초기식, 증감식이 존재하지 않고
	 * 	 반복 조건만을 설정하는 반복문
	 * 		
	 * - 반복 횟수가 정해져 있진 않지만
	 *   언젠간 반복이 종료되는 경우에 사용
	 * 
	 * [작성법]
	 * 
	 * while (조건식){
	 * 
	 * 		조건식이 true인 경우 반복 수행할 구문
	 * }
	 * 
	 * */
	
	public void ex1() {
		// while 기초 사용법 1
		
		// 1부터 10까지 출격 (while / for)
		System.out.println("=============for=============");
		for(int i = 1; i<=10; i++) {
			System.out.printf("%d ", i);
		}
		
		int i = 1;
		
		System.out.println("\n=============while=============");
		while(i<=10) {
			System.out.printf("%d ", i++);
		}
	}
	
	public void ex2() {
		// while 기초 사용법 2
		
		// 입력되는 모든 정수의 합 구하기
		
		Scanner sc = new Scanner(System.in);
		// 단, 0이 입력되면 반복 종료
		
		int sum = 0;
		int input = -1;			// -> 왜 0은 안되는가?
								// 	  0 으로 초기화 하는 경우 while문 조건이 바로 false가 되어
								//    한번도 반복을 못하는 상황이 발생하기 때문에
		
		while(input != 0) {
			// input cannot be resolved to a variable
			// == input 변수가 해석되지 않는다 == 변수가 존재하지 않는다.
			
			
			// The local variable input may not have been initialized
			// == input 변수가 초기화 되지 않았다.
			
			System.out.print("정수 입력 : ");
			input = sc.nextInt();
			
			sum += input;
			
			System.out.println("현재 누적 값 : " + sum);
		}
	
	}
//	public void ex2v2() {
//		int input = 0;
//		Scanner sc = new Scanner(System.in);
//		while() {
//			System.out.print("정수 입력 : ");
//			switch(input = sc.nextInt()) {
//			case 0 :
//				break;
//			}
//		}
//	}
	
	
	
	public void ex3() {
		// 분식집 주문 프로그램
		
		// ex)
		// --- 메뉴 ---
		// 1. 떡볶이(5000원)
		// 2. 김밥(3000원)
		// 3. 라면(4000원)
		// 4. 돈가스(8000원)
		// 9. 주문 완료
		// 메뉴 선택 >> 1

		// --- 메뉴 ---
		// 1. 떡볶이(5000원)
		// 2. 김밥(3000원)
		// 3. 라면(4000원)
		// 4. 돈가스(8000원)
		// 9. 주문 완료
		// 메뉴 선택 >> 2
		
		// --- 메뉴 ---
		// 1. 떡볶이(5000원)
		// 2. 김밥(3000원)
		// 3. 라면(4000원)
		// 4. 돈가스(8000원)
		// 9. 주문 완료
		// 메뉴 선택 >> 9
		
		// 떡볶이, 김밥 을 주문하셨습니다.
		// 총 가격은 8000원 입니다.
		
		
		Scanner sc = new Scanner(System.in);
		
		int input = 0;
		int totalPrice = 0;
		String menu = "";
		// "" == 빈 문자열 (내용은 없지만 문자열)
		
		boolean flag = false;		// 신호로 사용하느 변수
		// false : 첫 번째가 아닌 주문
		// true : 첫 번째 주문
		
		
		
		while(input != 9) {
			System.out.println("--- 메뉴 ---");
			System.out.println("1. 떡볶이(5000원)");
			System.out.println("2. 김밥(3000원)");
			System.out.println("3. 라면(4000원)");
			System.out.println("4. 돈가스(8000원)");
			System.out.println("9. 주문 완료");
			System.out.print("메뉴 선택 >> ");
			input = sc.nextInt();
			
			
			if(flag && input >= 1 && input <= 4) {	// true	첫 주문 x + 메뉴를 잘 입력 했을 때
				menu += ", ";
			} else {	// false 첫 주문인 경우
				flag = true;
			}
			
			switch(input) {
			case 1 : totalPrice += 5000; menu += "떡볶이"; break;
			case 2 : totalPrice += 3000; menu += "김밥"; break;
			case 3 : totalPrice += 4000; menu += "라면"; break;
			case 4 : totalPrice += 8000; menu += "돈가스"; break;
			case 9 : break;
			default : System.out.println("잘못 입력 하셨습니다");
			} // switch end
			System.out.println();
		} // while end
		System.out.printf("%s을/를 주문하셨습니다.", menu);
		System.out.printf("총 가격은 %d원 입니다.\n", totalPrice);
	}
	
	public void ex4() {
		
		/* do - while문 
		 * 
		 * [작성법]
		 * 
		 * do {
		 * 	
		 * 	- **최소 1회** 이상 반복 하는 while 문
		 * 
		 * } while(조건식);
		 * 
		 * do 			: { } 내부 코드를 일단 수행해라
		 * while(조건식) 	: 조건식이 true이면 { } 내부 코드를 반복 수행
		 * 
		 * */ 
		
		Scanner sc = new Scanner(System.in);
		
		int sum = 0;
		
		int input; 
		
		do {
			System.out.print("정수 입력 : ");
			input = sc.nextInt();
			
			sum += input;
			
			System.out.println("현재 누적 값 : " + sum);
		} while( input != 0 );
	}
}
